...
def expected_failures_renewal(t_val, mtbf):
    return t_val / mtbf

def expected_failures_nhpp(t_val, beta, lam, T_obs):
    n_total   = lam * (T_obs + t_val)**beta
    n_already = lam * T_obs**beta
    return float(n_total - n_already)
...
uploaded = st.file_uploader(
    "Upload engine failure data (.xlsx or .csv)",
    type=['xlsx', 'csv'],
...
if use_nhpp:
    ...
    # N(t) for NHPP
    st.markdown('<h2>Expected Failures N(t)</h2>', unsafe_allow_html=True)
    st.markdown(
        '<p class="note-box">For NHPP, N(t) depends on where you are on the test timeline. '
        'Enter a future duration t to get the expected failures in the next t hours '
        'beyond the current test end.</p>', unsafe_allow_html=True)

    col1, col2 = st.columns([1, 2])
    with col1:
        t_input = st.number_input(
            'Future duration t (hours)', min_value=0.1,
            max_value=10000.0, value=20.0, step=0.5,
            key='t_nhpp'
        )
    n_future = expected_failures_nhpp(t_input, beta, lam, T_obs)
    n_zero_to_t = lam * t_input**beta
    with col2:
        st.markdown(f"""
        <div class="metric-card">
          <div class="metric-label">Expected failures in next {t_input:.1f} hr
          (from T={T_obs:.2f} hr to T+t={T_obs+t_input:.2f} hr)</div>
          <div class="metric-value">{n_future:.3f}
            <span class="metric-unit"> ≈ {round(n_future)} failures</span></div>
          <div style="margin-top:10px" class="metric-label">
          Expected failures from t=0 to t={t_input:.1f} hr (for reference)</div>
          <div class="metric-value" style="font-size:1rem">{n_zero_to_t:.3f}
            <span class="metric-unit"> ≈ {round(n_zero_to_t)} failures</span></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown('<h2>Expected Failures N(t)</h2>', unsafe_allow_html=True)
    st.markdown(
        '<p class="note-box">For a stationary IID renewal process, the expected '
        'number of failures in any interval of duration t is t / MTBF, '
        'by the renewal reward theorem. This holds regardless of where in '
        'the timeline the interval starts.</p>', unsafe_allow_html=True)

    mtbf_best = best_f['mtbf']
    col1, col2 = st.columns([1, 2])
    with col1:
        t_input = st.number_input(
            'Time duration t (hours)', min_value=0.1,
            max_value=10000.0, value=10.0, step=0.5,
            key='t_iid'
        )
    n_exp = expected_failures_renewal(t_input, mtbf_best)
    with col2:
        st.markdown(f"""
        <div class="metric-card">
          <div class="metric-label">Expected failures in {t_input:.1f} hr
          using {best} MTBF = {mtbf_best:.4f} hr</div>
          <div class="metric-value">{n_exp:.4f}
            <span class="metric-unit"> ≈ {round(n_exp)} failures</span></div>
          <div style="color:#8b949e;font-size:0.78rem;margin-top:8px">
          N(t) = t / MTBF = {t_input:.2f} / {mtbf_best:.4f} = {n_exp:.4f}</div>
        </div>
        """, unsafe_allow_html=True)

    # Show all three engines' N(t) if user wants comparison
    with st.expander("Compare N(t) across all fitted distributions"):
        for name, f in fits.items():
            n = expected_failures_renewal(t_input, f['mtbf'])
            st.markdown(
                f'<div style="font-family:monospace;font-size:0.82rem;'
                f'color:#c9d1d9;padding:4px 0">'
                f'{name:12s}  MTBF={f["mtbf"]:.4f} hr  →  '
                f'N({t_input:.1f} hr) = {n:.4f}  ≈ {round(n)}</div>',
                unsafe_allow_html=True)
